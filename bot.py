...
BONUS_MIN_LEVEL = 5                    # فقط لولِ بالای این عدد واجدِ شرایطِ سوالِ ویژه‌ست
BONUS_HARD_LEVEL_THRESHOLD = 9         # از این لول به بالا (۹ و ۱۰)، سوال حتی سخت‌تر می‌شه
BONUS_INTERVAL_MIN_SECONDS = 8 * 3600  # ۸ ساعت
BONUS_INTERVAL_MAX_SECONDS = 12 * 3600  # ۱۲ ساعت
...
# ---------------------------------------------------------------------------
# سوال‌های ویژه: هر ۸ تا ۱۲ ساعت، فقط برای کسایی که لولِ اصلیشون بالای ۵ هست، یه سوالِ سخت‌تر
# می‌پرسه. جوابِ درست ۳۰ لولِ اصلی و ۲۰ امتیازِ درسِ ریاضی می‌ده (دو برابرِ سوالِ عادی).
# ---------------------------------------------------------------------------
def get_active_bonus_challenge(chat_id: int):
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT * FROM active_bonus_challenges WHERE chat_id=? AND resolved=0", (chat_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def save_active_bonus_challenge(chat_id: int, user_id: int, message_id: int, answer: int, question_text: str):
    ...
    c.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, 0) "
        "ON CONFLICT(chat_id) DO UPDATE SET user_id=excluded.user_id, message_id=excluded.message_id, "
...
def resolve_bonus_challenge(chat_id: int):
    conn = get_conn()
    c = conn.cursor()
    c.execute("UPDATE active_bonus_challenges SET resolved=1 WHERE chat_id=?", (chat_id,))
    conn.commit()
    conn.close()
...
def _pick_bonus_eligible_member(chat_id: int):
    """از بینِ اعضای حاضرِ گروه، یکی رو که لولِ اصلیش بالای BONUS_MIN_LEVEL هست، رندوم انتخاب
    می‌کنه. برمی‌گردونه (user_id, first_name, level) یا None اگه کسی واجدِ شرایط نبود."""
    conn = get_conn()
    c = conn.cursor()
    c.execute(
...
async def post_bonus_challenge(bot, chat_id: int) -> bool:
    if get_active_bonus_challenge(chat_id) is not None:
        return False
    picked = _pick_bonus_eligible_member(chat_id)
    if picked is None:
        return False
    user_id, first_name, level = picked
    ...
    text = (
        ...
        f"چون سطحت از {BONUS_MIN_LEVEL} بالاتره، یه سوالِ سخت‌تر برات داریم:\n\n"
        f"{question_text} = ?\n\n"
...
async def maybe_post_bonus_challenge_for_chat(bot, chat_id: int):
    ...
    if not settings["challenges_enabled"]:
        return
    active = get_active_bonus_challenge(chat_id)
    if active is not None:
        created_dt = _parse_iso(active["created_at"])
        if (datetime.utcnow() - created_dt).total_seconds() >= BONUS_CHALLENGE_LIFETIME_SECONDS:
            resolve_bonus_challenge(chat_id)
            try:
                await bot.delete_message(chat_id, active["message_id"])
            except Exception:
                pass
    ...
    if now_ts < next_at:
        return
    posted = await post_bonus_challenge(bot, chat_id)
    # چه پست بشه چه کسی واجدِ شرایط نباشه، نوبتِ بعدی رو عقب می‌ندازیم تا هر پیام دوباره امتحان نکنه
    new_next = now_ts + random.randint(BONUS_INTERVAL_MIN_SECONDS, BONUS_INTERVAL_MAX_SECONDS)
    set_next_bonus_challenge_at(chat_id, new_next)
    return posted
