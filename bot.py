...
async def group_member_update_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.new_chat_members:
        for member in msg.new_chat_members:
            ...
            text = (
                f"سلام {mention} عزیز 🌸\n"
                f"به گروه {html.escape(chat_title)} خوش آمدید\n\n"
...
# ---------------------------------------------------------------------------
def _install_universal_message_tracking(app: Application):
    """
    قابلیت «حذف پیام‌ها» فقط پیام‌هایی رو می‌تونه پاک کنه که تو جدولِ tracked_messages ثبت
    شده باشن. پیام‌های کاربرها و ربات‌های دیگه از راهِ آپدیت‌های ورودی خودکار ثبت می‌شن
    (group_catch_all_tracker)، ولی پیام‌های خودِ این ربات فقط از همون‌جایی ثبت می‌شن که صراحتاً
    track_message() صدا زده باشه - و خیلی جاهای کد (تو خودِ bot.py و تو dooz_game/rps_game/
    chess_game) این کارو فراموش کرده بودن. به‌جای پیدا کردنِ تک‌تکِ اون جاها، همینجا سطحِ
    پایین‌تر (خودِ متدهای ارسالِ bot) رو یه‌بار پچ می‌کنیم تا هر پیامی که به یه گروه فرستاده
    بشه - از هر جای کد، الان یا تو آینده - خودکار ثبت بشه.
    """
    def _wrap(method_name):
        original = getattr(app.bot, method_name, None)
        if original is None:
            return
        ...
        async def wrapped(*args, **kwargs):
            ...
            try:
                ...
                if chat_id is not None and int(chat_id) < 0 and result is not None:
                    mid = getattr(result, "message_id", None)
                    if mid:
                        track_message(int(chat_id), mid)
            except Exception:
                pass
            return result
    ...
    for method_name in (
        ...
        "send_document", "send_voice", "send_audio", "send_sticker", "send_poll",
    ):
        _wrap(method_name)
...
def build_application() -> Application:
    ...
    _install_universal_message_tracking(app)
    ...
    # ---- منوی مشترکِ «بازی» (انتخاب بین دوز، سنگ‌کاغذقیچی، و شطرنج) ------------
