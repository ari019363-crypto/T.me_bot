...
async def challenge_scheduler_job(context: ContextTypes.DEFAULT_TYPE):
    """هر چند دقیقه یه‌بار چک می‌کنه کدوم گروه‌ها نوبتِ چالشِ بعدیشونه؛ اگه رسیده بود، یه چالشِ
    جدید می‌فرسته و نوبتِ بعدی رو با یه فاصله‌ی رندومِ ۱۰ تا ۲۰ دقیقه‌ای تنظیم می‌کنه."""
    now_ts = int(datetime.utcnow().timestamp())
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT chat_id, next_challenge_at FROM group_settings WHERE challenges_enabled=1")
    rows = c.fetchall()
    conn.close()
    for row in rows:
        chat_id = row["chat_id"]
        next_at = row["next_challenge_at"] or 0
        if now_ts < next_at:
            continue
        posted = await post_new_challenge(context.bot, chat_id)
        if posted:
...
def main():
    ...
    if app.job_queue is not None:
        ...
        # هر ۲ دقیقه چک می‌کنه کدوم گروه‌ها نوبتِ چالشِ ریاضیِ بعدیشونه (فاصله‌ی هر چالش ۱۰ تا ۲۰ دقیقه‌ست)
        app.job_queue.run_repeating(challenge_scheduler_job, interval=120, first=90)
    else:
        logger.warning(
