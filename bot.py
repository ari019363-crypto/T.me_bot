...
def init_db():
    ...
    conn.commit()
    # اگه یه ادمین وبِ همین حالا داره بات رو خاموش نگه می‌داره، این مقدار رو دست نمی‌زنیم؛ فقط
    # مطمئن می‌شیم ردیفِ کنترل وجود داره تا پنل بتونه ازش بخونه/بنویسه.
...
def is_bot_paused() -> bool:
    """پنل تحت وب با نوشتنِ این فلگ می‌تونه ربات رو موقتاً «خاموشِ نرم» کنه، بدون این‌که
    پروسه واقعاً بمیره یا هیچ دیتایی از دست بره - فقط پردازش پیام‌ها متوقف می‌شه."""
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT value FROM bot_control WHERE key='paused'")
    row = c.fetchone()
    conn.close()
    return bool(row and row["value"] == "1")


def touch_heartbeat(mark_started: bool = False):
    conn = get_conn()
    c = conn.cursor()
    now = datetime.utcnow().isoformat()
    if mark_started:
        c.execute(
            "INSERT INTO bot_status (id, started_at, last_heartbeat) VALUES (1, ?, ?) "
...
def log_mod_action(chat_id: int, actor_id: int, actor_name: str, action: str, target_id: int = None, target_name: str = "", reason: str = ""):
    """هر اقدام مدیریتی (بن/اخطار/سکوت/آزاد) رو ثبت می‌کنه تا تو پنل وب، بخشِ «لاگ» دیده بشه."""
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        "INSERT INTO mod_logs (chat_id, actor_id, actor_name, action, target_id, target_name, reason, created_at) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (chat_id, actor_id, actor_name, action, target_id, target_name, reason, datetime.utcnow().isoformat()),
    )
    conn.commit()
    conn.close()
...
def build_application() -> Application:
    ...
    # ---- گیت خاموش/روشنِ نرم (پنل تحت وب کنترلش می‌کنه) --------------------
    # اگه ادمین از پنل وب ربات رو «خاموش» کنه، این هندلر (که تو زودترین گروه اجرا می‌شه)
    # جلوی پردازش هر پیام/دکمه‌ای رو می‌گیره - پروسه زنده می‌مونه و هیچ دیتایی از دست نمی‌ره،
    # فقط ربات موقتاً ساکت می‌شه. با روشن‌کردنِ دوباره از پنل، همه‌چیز دقیقاً از همون‌جا ادامه پیدا می‌کنه.
    async def _pause_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
        if is_bot_paused():
            raise ApplicationHandlerStop

    app.add_handler(MessageHandler(filters.ALL, _pause_gate), group=-1)
    app.add_handler(CallbackQueryHandler(_pause_gate, pattern=".*"), group=-1)

    # ---- ضربان قلب برای پنل وب (وضعیت آنلاین/آفلاین و مدت روشن‌بودن) -------
    async def _heartbeat_job(context: ContextTypes.DEFAULT_TYPE):
        touch_heartbeat()

    async def _startup(app_: Application):
        touch_heartbeat(mark_started=True)
