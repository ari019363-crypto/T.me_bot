...
DAILY_COIN_GIFT = 2          # سهم هرکسی که تو گروه حضور داره
DAILY_SPECIAL_PRIZE = 4      # سکه‌ی اضافه‌ای که فقط برنده‌ی رندومِ هر شب می‌گیره
SPAM_WINDOW_SECONDS = 2.0    # این تعداد پیام باید داخل همین بازه‌ی زمانی بیاد تا اسپم حساب بشه
...
async def daily_coin_job(context: ContextTypes.DEFAULT_TYPE):
    """هر شب دقیقاً نصفه‌شب (به وقت تهران) به همه‌ی اعضای حاضرِ هر گروه چندتا سکه می‌ده و یه نفر رو
    هم به‌صورت رندوم به‌عنوان جایزه‌ی ویژه‌ی اون شب انتخاب می‌کنه (سکه‌ی بیشتر)."""
    conn = get_conn()
    c = conn.cursor()
    c.execute("SELECT chat_id FROM group_settings")
    ...
    for chat_id in chat_ids:
        ...
        if not members:
            continue
        for m in members:
            penalty_game.add_coins(m["user_id"], DAILY_COIN_GIFT)
        winner = random.choice(members)
        penalty_game.add_coins(winner["user_id"], DAILY_SPECIAL_PRIZE)
        name_escaped = html.escape(winner["first_name"] or "یکی از بچه‌ها")
        mention = f'<a href="tg://user?id={winner["user_id"]}">{name_escaped}</a>'
        text = (
            f"🌙 نصفه‌شب شد! کل {len(members)} عضو {DAILY_COIN_GIFT} سکه دریافت کردن. 🪙\n"
            f"🎁 جایزه‌ی ویژه‌ی امروز هم می‌رسه به {mention}! (+{DAILY_SPECIAL_PRIZE} سکه‌ی اضافه)\n\n"
            f"می‌تونید از این سکه‌ها تو بازی پنالتی ⚽ استفاده کنید."
        )
        try:
...
def main():
    ...
    if app.job_queue is not None:
        ...
        # دقیقاً نصفه‌شب (۰۰:۰۰) به وقت تهران، سکه‌ی روزانه رو پخش می‌کنه
        app.job_queue.run_daily(daily_coin_job, time=dt_time(hour=0, minute=0, second=0, tzinfo=TEHRAN_TZ))
    else:
        logger.warning(
