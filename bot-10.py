...
async def group_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # "بگو <متن>" - ربات همون متن رو (بدون خودِ کلمه‌ی «بگو») می‌فرسته
    ...
    # "چت <متن>" - پیامِ خودِ فرستنده حذف می‌شه و متن به‌جاش از طرفِ ربات ارسال می‌شه
    # (فقط ادمین؛ چون این یعنی می‌شه چیزی رو به اسمِ ربات جا زد، بهتره دستِ همه نباشه)
    if text_stripped.startswith("چت "):
        if not await is_user_admin(update, context, user.id, chat_id):
            await reply_tracked(msg, "⛔ این قابلیت فقط برای ادمین‌های گروهه.")
            return
