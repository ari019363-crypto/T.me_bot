...
def init_db():
    ...
    # مهاجرتِ دفاعی: اگه جدولِ active_bonus_challenges از نسخه‌ی قبلی با کلیدِ اصلیِ تک‌ستونی
    # (فقط chat_id) مونده باشه، اینجا شناساییش می‌کنیم و از نو می‌سازیمش با کلیدِ ترکیبیِ درست
    # (chat_id, user_id). دیتای این جدول کاملاً موقتیه (فقط چالش‌های درحال‌انتظار)، پس پاک شدنش
    # هیچ آسیبی نمی‌زنه.
    try:
        c.execute("PRAGMA table_info(active_bonus_challenges)")
        cols = c.fetchall()
        if cols:
            pk_cols = {col["name"] for col in cols if col["pk"] > 0}
            if pk_cols != {"chat_id", "user_id"}:
                c.execute("DROP TABLE active_bonus_challenges")
                conn.commit()
    except Exception:
        pass
...
def save_active_bonus_challenge(chat_id: int, user_id: int, message_id: int, answer: int, question_text: str):
    """نکته‌ی مهم: عمداً از INSERT OR REPLACE استفاده می‌کنیم، نه ON CONFLICT(chat_id, user_id).
    چون اگه دیتابیسِ زنده از قبل (از نسخه‌ی قبلی) این جدول رو با کلیدِ اصلیِ تک‌ستونی (فقط chat_id)
    ساخته باشه، اون ON CONFLICT دقیقاً رو همون دو ستون match نمی‌شد و کل INSERT با خطا مواجه
    می‌شد - یعنی پیامِ سوال فرستاده می‌شد ولی تو دیتابیس ثبت نمی‌شد، پس جوابِ کاربر هیچ‌وقت پیدا
    نمی‌شد. INSERT OR REPLACE با هر شکلی از کلیدِ یکتا (چه تک‌ستونی چه ترکیبی) درست کار می‌کنه."""
    now_iso = datetime.utcnow().isoformat()
    conn = get_conn()
    c = conn.cursor()
    try:
        c.execute(
            "INSERT OR REPLACE INTO active_bonus_challenges "
        ...
        logger.warning("⚠️ نتونستم چالشِ ویژه رو تو دیتابیس ذخیره کنم؛ جدولِ active_bonus_challenges احتمالاً ساختارِ قدیمی داره.")
    conn.close()
