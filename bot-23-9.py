...
CHALLENGE_INTERVAL_MIN_SECONDS = 600   # ۱۰ دقیقه
CHALLENGE_INTERVAL_MAX_SECONDS = 1200  # ۲۰ دقیقه
CHALLENGE_CLEANUP_DELAY_SECONDS = 300  # ۵ دقیقه؛ بعدش پیام‌های چالش پاک می‌شن
CHALLENGE_EXPIRE_SECONDS = 300  # اگه ۵ دقیقه کسی جواب نده، چالش منقضی می‌شه تا جلوی بعدی رو نگیره
...
def save_active_challenge(chat_id: int, message_id: int, answer: int):
    conn = get_conn()
    c = conn.cursor()
    c.execute(
        "INSERT INTO active_challenges (chat_id, message_id, answer, created_at, resolved) VALUES (?, ?, ?, ?, 0) "
        "ON CONFLICT(chat_id) DO UPDATE SET message_id=excluded.message_id, answer=excluded.answer, "
        "created_at=excluded.created_at, resolved=0",
        (chat_id, message_id, answer, datetime.utcnow().isoformat()),
    )
    conn.commit()
    conn.close()
...
async def expire_stale_challenge_if_needed(bot, active) -> bool:
    """اگه یه چالش خیلی وقته (بیشتر از CHALLENGE_EXPIRE_SECONDS) بی‌جواب مونده، منقضیش می‌کنه
    (پیامش رو ادیت می‌کنه که وقت تموم شده) تا جلوی فرستادنِ چالشِ بعدی رو نگیره.
    این دقیقاً همون چیزیه که قبلاً نبود: بدونِ این تابع، یه چالشِ بی‌جواب برای همیشه «فعال» می‌موند
    و کلِ زمان‌بندی رو برای اون گروه قفل می‌کرد."""
    try:
        created_dt = datetime.fromisoformat(active["created_at"])
    except (TypeError, ValueError):
        created_dt = datetime.utcnow()
    age_seconds = (datetime.utcnow() - created_dt).total_seconds()
    if age_seconds < CHALLENGE_EXPIRE_SECONDS:
        return False
    resolve_challenge(active["chat_id"])
    try:
        await bot.edit_message_text(
            "🧮 چالشِ ریاضی!\n\n⌛️ وقت تموم شد و کسی جوابِ درست رو نداد. چالشِ بعدی به‌زودی میاد!",
...
def level_from_xp(xp: int):
    """برمی‌گردونه (سطح, آیا_افسانه‌ای). هر ۱۰۰ اکسپی یه سطح؛ از ۱۰۰۰ به بعد «افسانه‌ای»."""
    if xp >= 1000:
        return 10, True
    return 1 + (xp // 100), False
...
def add_subject_correct(user_id: int, subject: str):
    ...
    if row is None:
        c.execute(
            "INSERT INTO user_subject_levels (user_id, subject, correct_count) VALUES (?, ?, ?)",
            (user_id, subject, new_count),
        )
    else:
        c.execute(
            "UPDATE user_subject_levels SET correct_count=? WHERE user_id=? AND subject=?",
            (new_count, user_id, subject),
        )
    conn.commit()
    conn.close()
...
async def challenge_cleanup_job(context: ContextTypes.DEFAULT_TYPE):
    """۵ دقیقه بعد از حل شدنِ چالش، پیامِ چالش و پیامِ جوابِ کاربر رو پاک می‌کنه تا گپ تمیز بمونه."""
...
async def post_new_challenge(bot, chat_id: int) -> bool:
    ...
    اگه چالشِ قبلی هنوز «فعال» نشون داده می‌شه ولی خیلی وقته کسی جوابش رو نداده، اول منقضیش
    می‌کنیم تا برای همیشه جلوی چالش‌های بعدی رو نگیره."""
    active = get_active_challenge(chat_id)
    if active is not None:
        expired = await expire_stale_challenge_if_needed(bot, active)
        if not expired:
            return False
    ...
    save_active_challenge(chat_id, sent.message_id, answer)
    return True


async def maybe_post_challenge_for_chat(bot, chat_id: int):
    """چک می‌کنه آیا الان نوبتِ چالشِ بعدیِ این گروهه؛ اگه بله، یه چالشِ جدید می‌فرسته و نوبتِ
    بعدی رو با یه فاصله‌ی رندومِ ۱۰ تا ۲۰ دقیقه‌ای تنظیم می‌کنه.
    این تابع از دو تا مسیرِ کاملاً مستقل صدا زده می‌شه: هم از جاب‌کیو (هر ۶۰ ثانیه)، هم از دلِ
    پردازشِ پیام‌های عادیِ گروه. این‌جوری حتی اگه جاب‌کیو رو سرور فعال نباشه (مثلاً پکیجِ
    python-telegram-bot[job-queue] نصب نشده باشه)، تا وقتی تو گروه پیام رد و بدل می‌شه، چالش‌ها
    باز هم سرِ وقتِ خودشون می‌رسن - چون به ساعتِ خودِ سرور (که با NTP همیشه دقیقه) وابسته‌ست، نه
    به جاب‌کیو یا هیچ سرویسِ بیرونی‌ای."""
    ...
    c = conn.cursor()
    if posted:
        new_next = now_ts + random.randint(CHALLENGE_INTERVAL_MIN_SECONDS, CHALLENGE_INTERVAL_MAX_SECONDS)
    else:
        # یه چالشِ حل‌نشده‌ی قبلی هنوز بازه؛ یه‌کم دیرتر دوباره امتحان کن (نه هر پیام)
        new_next = now_ts + 60
    c.execute("UPDATE group_settings SET next_challenge_at=? WHERE chat_id=?", (new_next, chat_id))
    conn.commit()
    conn.close()
...
def level_up_message_text(mention: str, old_level: int, new_level: int, is_legendary: bool) -> str:
    if is_legendary:
        return f"👑🔥 وایییی {mention} رسید به مقامِ افسانه‌ای! از این به بعد باید باهاش رسمی‌تر حرف بزنیم 😌👑"
    return (
        f"🎉✨ به‌به {mention}!\n"
        f"رفتی رو دنده‌ی تند؛ از سطح {old_level} پریدی رو سطح {new_level} 🚀\n"
        "همینجوری ادامه بده، داری نخبه‌ی گروه می‌شی 😎"
    )
...
async def challenge_answer_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if context.job_queue is not None:
            context.job_queue.run_once(
                challenge_cleanup_job, 20, data={"chat_id": chat_id, "message_ids": [wrong_msg.message_id, msg.message_id]}
            )
        return
    ...
    _, new_math_count = add_subject_correct(user.id, "ریاضی")
    ...
    if context.job_queue is not None:
        context.job_queue.run_once(
            ...
            CHALLENGE_CLEANUP_DELAY_SECONDS,
...
async def adm_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton(f"🧮 چالش‌های سرگرمی: {_toggle_col(s, 'challenges_enabled')}", callback_data="set_toggle_challenges")],
...
async def set_toggle_challenges(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _toggle_setting_and_refresh(update, context, "challenges_enabled")
...
def build_application() -> Application:
    ...
    # این باید با group=-1 ثبت بشه (اولویت بالاتر از group_message_handler که تو group=0
    # پیش‌فرضه) وگرنه چون اون هندلرِ عمومی فیلترش هر پیامِ متنیِ گروه رو می‌گیره، این هیچ‌وقت اجرا نمی‌شه.
    app.add_handler(
