...
async def pen_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"👤 پروفایل پنالتیِ {user.first_name or 'بازیکن'}", "", f"🪙 سکه: {coins}"]
    if stats is None:
        lines.append("")
...
async def penalty_challenge_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE, forced_rounds: int = 5):
    ...
    challenger_name = challenger.first_name or "بازیکن ۱"
    opponent_name = opponent.first_name or "بازیکن ۲"
