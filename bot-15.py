...
# ---------------------------------------------------------------------------
async def pv_chat_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    ...
    if len(groups) == 1:
        ...
        await update.message.reply_text(
            ...
            "بدون این‌که خودِ این پیام هیچ‌وقت تو گروه بره."
        )
    elif len(groups) > 1:
        keyboard = InlineKeyboardMarkup(
...
async def pv_chat_unlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if get_pv_link(user_id):
        remove_pv_link(user_id)
        await update.message.reply_text("🔌 لینک قطع شد.")
    else:
        await update.message.reply_text("لینکی وصل نبود.")


async def pv_chat_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """«چت <متن>» تو پیوی - متن رو بی‌سروصدا تو گروهِ لینک‌شده پست می‌کنه."""
    user_id = update.effective_user.id
    say_text = update.message.text.strip()[len("چت "):].strip()
    link = get_pv_link(user_id)
    if not link:
        await update.message.reply_text("اول باید وصل بشی؛ فقط بنویس «چت» تا شروع کنیم.")
        return
    chat_id = link["chat_id"]
    if not await is_user_admin(update, context, user_id, chat_id):
        remove_pv_link(user_id)
        await update.message.reply_text(f"⛔ دیگه تو «{link['chat_title']}» ادمین نیستی، لینک قطع شد.")
        return
    if not say_text:
        await update.message.reply_text("بعد از «چت» متنی که می‌خوای فرستاده بشه رو هم بنویس.")
        return
    # چون این پیام تو پیویِ خودِ همون آدمه، پیام‌رسانیِ فوری لازم نیست (کسِ دیگه‌ای نمی‌بینتش)؛
    # فقط برای تمیزی، خودِ پیامِ «چت ...» رو از تاریخچه‌ی همین پیوی پاک می‌کنیم.
    asyncio.create_task(_safe_delete_message(context, update.effective_chat.id, update.message.message_id))
    try:
        sent = await context.bot.send_message(chat_id, say_text)
        track_message(chat_id, sent.message_id)
...
def build_application() -> Application:
    ...
    # ---- «چت» از طریق پیوی -----------------------------------------------
    app.add_handler(MessageHandler(filters.Regex(r"^چت$") & filters.ChatType.PRIVATE, pv_chat_entry))
    app.add_handler(MessageHandler(filters.Regex(r"^چت جدا شو$") & filters.ChatType.PRIVATE, pv_chat_unlink))
    app.add_handler(MessageHandler(filters.Regex(r"^چت\s+.+") & filters.ChatType.PRIVATE, pv_chat_send))
